...
davis_path = 'F:\\data\\DAVIS-data\\DAVIS\\JPEGImages\\480p'
setting = './result/test/davis_iteration_conf_gnn3_sa_org_scale_batch/Results'
out_folder = './result/davis-crf/'
for d in listdir(setting):

    vidDir = join(davis_path, d)   # /davis_Path/blackswan
    resDir = join(out_folder, d)
    if not os.path.exists(resDir):
        os.makedirs(resDir)
    for f in listdir(vidDir): # f 是分割的每一帧的名字
